...
import { annualRecords } from '../data';
import { useParams } from 'next/navigation';
...
export default function DonationRecordDetail () {
	...
	const recordDetails = annualRecords.find(record => record.id === id);
	const currentIndex = annualRecords.findIndex(record => record.id === id);

	const prevRecord = annualRecords[currentIndex + 1];
	const nextRecord = annualRecords[currentIndex - 1];
					...
					<div className="border-t border-b border-gray-200">
						<div className="flex border-b border-gray-200">
							...
							<div className="flex-1 py-4 px-4">
								{nextRecord ? (
									<Link href={`/donation/annual-records/${nextRecord.id}`} className="text-gray-800 hover:underline">
										{nextRecord.title}
									</Link>
								) : (
						...
						<div className="flex">
							...
							<div className="flex-1 py-4 px-4">
								{prevRecord ? (
									<Link href={`/donation/annual-records/${prevRecord.id}`} className="text-gray-800 hover:underline">
										{prevRecord.title}
									</Link>
								) : (
